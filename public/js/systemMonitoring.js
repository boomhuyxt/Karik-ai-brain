...
(function () {
    ...
    window.switchMonitorTab = function (tabKey) {
        ...
        // Tab Buttons
        const tabs = [
            { key: 'token', btnId: 'tabBtnToken', contentId: 'tabContentToken', activeClass: 'bg-cyan-500/20 text-cyan-300 border-cyan-400/40' },
            { key: 'progress', btnId: 'tabBtnProgress', contentId: 'tabContentProgress', activeClass: 'bg-purple-500/20 text-purple-300 border-purple-400/40' },
            { key: 'apirisk', btnId: 'tabBtnApiRisk', contentId: 'tabContentApiRisk', activeClass: 'bg-emerald-500/20 text-emerald-300 border-emerald-400/40' }
        ];
        ...
        tabs.forEach(t => {
            ...
            if (t.key === tabKey) {
                if (btn) {
                    btn.className = `monitor-tab-btn active px-4 py-2 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all border shadow-lg ${t.activeClass}`;
                }
                if (content) content.classList.remove('hidden');
            } else {
                if (btn) {
                    btn.className = 'monitor-tab-btn px-4 py-2 rounded-xl text-xs font-semibold flex items-center gap-2 transition-all text-slate-400 hover:text-slate-200 hover:bg-white/5 border border-transparent';
                }
    ...
    function renderTokenAnalytics(tokenData) {
        ...
        if (agentList && Array.isArray(tokenData.agents)) {
            agentList.innerHTML = tokenData.agents.map(ag => `
                <div class="p-3.5 rounded-xl bg-white/[0.03] border border-white/10 flex flex-col gap-2.5">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <span class="w-2.5 h-2.5 rounded-full" style="background-color: ${ag.color}"></span>
                            <span class="text-xs font-bold text-white">${ag.name}</span>
                            <span class="text-[9px] font-mono px-1.5 py-0.2 rounded bg-slate-800 text-slate-300 border border-slate-700">${ag.model}</span>
                        </div>
                        <div class="text-right font-mono">
                            <span class="text-xs font-bold text-white">${ag.usedTokens.toLocaleString()}</span>
                            <span class="text-[10px] text-slate-400 ml-0.5">Tokens (${ag.sharePct}%)</span>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between text-[10px] text-slate-400 font-mono">
                        <span>Input: <strong>${ag.inputTokens.toLocaleString()}</strong> Tokens</span>
                        <span>Output: <strong>${ag.outputTokens.toLocaleString()}</strong> Tokens</span>
                        <span class="text-emerald-400">Trạng thái: 🟢 Sẵn sàng</span>
                    </div>
                </div>
            `).join('');
        }
            ...
            providerList.innerHTML = tokenData.providers.map(p => `
                <div class="p-3 rounded-xl bg-white/[0.02] border border-white/5 flex items-center justify-between">
                    <div>
                        ...
                        <span class="text-[10px] text-slate-400 font-mono">Dung lượng: ${p.used.toLocaleString()} / ${p.max.toLocaleString()}</span>
                    </div>
                    <div class="text-right">
                        <span class="text-xs font-bold text-white font-mono">${p.percentage}%</span>
                        <span class="text-[9px] block ${p.isLinked ? 'text-emerald-400' : 'text-slate-500'} font-mono">
    ...
    function renderProjectProgress(projData) {
        ...
        if (grid && Array.isArray(projData.campaigns)) {
            grid.innerHTML = projData.campaigns.map(camp => `
                <div class="p-4 rounded-xl bg-slate-900/70 border border-white/10 flex flex-col justify-between gap-3 transition-all hover:border-cyan-500/30">
                    <div>
                        <div class="flex items-center justify-between mb-1.5">
                            <span class="text-[10px] font-mono px-2 py-0.5 rounded bg-cyan-500/15 text-cyan-300 border border-cyan-400/30">
                                ${camp.category}
                            </span>
                        ...
                        <h4 class="text-xs font-bold text-white leading-snug">${camp.name}</h4>
                        <span class="text-[10px] text-slate-400 block mt-1">Phụ trách: <strong class="text-slate-200">${camp.assignedAgent}</strong></span>
                    </div>
                    ...
                    <div class="grid grid-cols-3 gap-2 p-2 rounded-lg bg-black/40 border border-white/5 text-center text-[10px] font-mono">
                        ${camp.metrics.ctr ? `
                            <div>
                                <span class="text-slate-500 block">CTR</span>
                                <span class="font-bold text-emerald-400">${camp.metrics.ctr}</span>
                            </div>
                            <div>
                                <span class="text-slate-500 block">CPC</span>
                                <span class="font-bold text-cyan-400">${camp.metrics.cpc}</span>
                            </div>
                            <div>
                                <span class="text-slate-500 block">ROAS</span>
                                <span class="font-bold text-amber-400">${camp.metrics.roas}</span>
                            </div>
                        ` : `
                            <div>
                                <span class="text-slate-500 block">Notes</span>
                                <span class="font-bold text-purple-400">${camp.metrics.totalFiles || camp.metrics.variantsCreated || 0}</span>
                            </div>
                            <div>
                                <span class="text-slate-500 block">Độ khớp</span>
                                <span class="font-bold text-emerald-400">${camp.metrics.syncRate || camp.metrics.ctrEstimated || '100%'}</span>
                            </div>
                            <div>
                                <span class="text-slate-500 block">Rủi ro</span>
                                <span class="font-bold text-emerald-400">${camp.risk.level} (${camp.risk.score}%)</span>
                            </div>
                        `}
                    </div>
                    ...
                    <div class="flex items-center gap-1.5 text-[10px] text-slate-400">
                        <span class="material-symbols-outlined text-xs text-emerald-400">shield</span>
    ...
    function renderApiRiskMatrix(apiData) {
        ...
        if (summaryText) {
            ...
            summaryText.style.color = apiData.overallColor || '#34d399';
        }
        if (connectedText) {
        ...
        if (matrixList && Array.isArray(apiData.apis)) {
            matrixList.innerHTML = apiData.apis.map(api => `
                <div class="p-4 rounded-xl bg-slate-900/70 border border-white/10 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 transition-all hover:border-white/20">
                    <div class="flex items-start gap-3 min-w-[280px]">
                        <div class="w-8 h-8 rounded-lg flex items-center justify-center mt-0.5 ${
                            api.status === 'connected' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-400/30' : 'bg-amber-500/20 text-amber-400 border border-amber-400/30'
                        }">
                            <span class="material-symbols-outlined text-lg">${api.status === 'connected' ? 'check_circle' : 'warning'}</span>
                        </div>
                        <div>
                            <div class="flex items-center gap-2">
                                <h4 class="text-xs font-bold text-white">${api.name}</h4>
                                <span class="text-[9px] font-mono px-1.5 py-0.2 rounded bg-slate-800 text-slate-300 border border-slate-700">${api.type}</span>
                            </div>
                            <p class="text-[11px] text-slate-400 mt-0.5">${api.detail}</p>
                            <span class="text-[10px] text-cyan-400/90 font-mono mt-1 block">Khuyến nghị: ${api.recommendation}</span>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-4 self-end md:self-center font-mono text-right">
                        <div>
                            ...
                            <span class="text-xs font-bold text-slate-200">${api.latency}</span>
                        </div>
                        <div>
                            <span class="text-[10px] text-slate-500 block">Mức độ rủi ro</span>
                            <span class="text-xs font-bold ${api.riskScore < 20 ? 'text-emerald-400' : (api.riskScore < 50 ? 'text-amber-400' : 'text-red-400')}">
                                ${api.riskLevel} (${api.riskScore}%)
                            </span>
                        </div>
                        <div class="w-20 bg-slate-800 rounded-full h-1.5 overflow-hidden border border-white/5">
                            <div class="h-full rounded-full ${api.riskScore < 20 ? 'bg-emerald-400' : 'bg-amber-400'}" style="width: ${api.riskScore}%"></div>
                        </div>
                    </div>
                </div>
            `).join('');
